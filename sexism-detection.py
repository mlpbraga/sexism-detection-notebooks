...
def generate_cross_validation_train_and_test(df):
    ...
    for i in range(0, 10):
        ...
        sexist_vocabulary, sexist_frequency_array, sexist_word_list = get_vocabulary(
            sexist_comments)
        sexist_bigrams, sexist_frequency_array, sexist_word_list = get_bigrams(
            sexist_comments)

        not_sexist_comments = dataframe[dataframe['avg'] < 0.5]
        not_sexist_vocabulary, not_sexist_frequency_array, not_sexist_word_list = get_vocabulary(
            not_sexist_comments)
        not_sexist_bigrams, not_sexist_frequency_array, not_sexist_word_list = get_bigrams(
            not_sexist_comments)

        shared_unigrams = select_shared_terms(
            sexist_vocabulary, not_sexist_vocabulary)
        sexist_unigrams, not_sexist_unigrams = select_word_freq(
            shared_unigrams, sexist_vocabulary, not_sexist_vocabulary)

        shared_bigrams = select_shared_terms(
            sexist_bigrams, not_sexist_bigrams)
        sexist_bigrams, not_sexist_bigrams = select_word_freq(
            shared_bigrams, sexist_bigrams, not_sexist_bigrams)
        ...
        not_sexist_doc = get_bigram_doc(
            not_sexist_comments, relevant_sexist_bigrams)
        ...
        tf_sexist_bigrams_dataframe = pd.concat(
        ...
        not_sexist_doc = get_doc(
            not_sexist_comments, relevant_not_sexist_words)
        sexist_tf = pd.DataFrame(
            sexist_vectorizer.fit_transform(sexist_doc).toarray())
        not_sexist_tf = pd.DataFrame(
            not_sexist_vectorizer.fit_transform(not_sexist_doc).toarray())
        tf_not_sexist_dataframe = pd.concat(
            [sexist_tf, not_sexist_tf]).fillna(0)
        tf_not_sexist_dataframe.columns = [f'TFun_{i}' for i in range(100)]

        # ------ not sexist bigrams TF
        sexist_doc = get_bigram_doc(
            sexist_comments, relevant_not_sexist_bigrams)
        not_sexist_doc = get_bigram_doc(
            not_sexist_comments, relevant_not_sexist_bigrams)
        sexist_bigrams_tf = pd.DataFrame(
            sexist_bigrams_vectorizer.fit_transform(sexist_doc).toarray())
        not_sexist_bigrams_tf = pd.DataFrame(
            not_sexist_bigrams_vectorizer.fit_transform(not_sexist_doc).toarray())
        tf_not_sexist_bigrams_dataframe = pd.concat(
            [sexist_bigrams_tf, not_sexist_bigrams_tf]).fillna(0)
        tf_not_sexist_bigrams_dataframe.columns = [
        ...
        likes_df = np.array(
            pd.concat([sexist_comments['likes'], not_sexist_comments['likes']]).fillna(0))
        dislikes_df = np.array(pd.concat(
            [sexist_comments['dislikes'], not_sexist_comments['dislikes']]).fillna(0))
        char_qty_df = np.array(pd.concat(
            [sexist_comments['char-qty'], not_sexist_comments['char-qty']]).fillna(0))
        word_qty_df = np.array(pd.concat(
            [sexist_comments['word-qty'], not_sexist_comments['word-qty']]).fillna(0))
...
def select_df_columns(df, columns):
    list_features = [
        df[df.columns[list(df.columns).index(x)]] for x in columns]
...
for i in tqdm(range(0, 1)):
    ...
    feature_combinations = {
        # sexist unigrams TFs
        'TFus': [x for x in list(train.columns) if 'TFus' in x],
        # not-sexist unigrams TFs
        'TFun': [x for x in list(train.columns) if 'TFun' in x],
        # sexist bigrams TFs
        'TFbs': [x for x in list(train.columns) if 'TFbs' in x],
        # not sexist bigrams TFs
        'TFbn': [x for x in list(train.columns) if 'TFbn' in x],
        'L': ['likes'],  # likes quantity
        'D': ['dislikes'],  # dislikes quantity
        'CQ': ['char-qty'],  # chars quantity
        'WQ': ['word-qty']  # word quantity
    }
    feature_combinations['LD'] = feature_combinations['L'] + \
        feature_combinations['D']
    feature_combinations['TFu'] = feature_combinations['TFus'] + \
        feature_combinations['TFun']
    feature_combinations['TFb'] = feature_combinations['TFbs'] + \
        feature_combinations['TFbn']
    feature_combinations['TF'] = feature_combinations['TFu'] + \
        feature_combinations['TFb']
    feature_combinations['CQWQ'] = feature_combinations['CQ'] + \
        feature_combinations['WQ']
    feature_combinations['Q'] = feature_combinations['LD'] + \
        feature_combinations['CQWQ']
    feature_combinations['TFuQ'] = feature_combinations['TFu'] + \
        feature_combinations['Q']

    combination_results = dict({})

    for combination in tqdm(feature_combinations.keys()):
        X_train_ = select_df_columns(
            X_train, feature_combinations[combination])
        ...
        precision, recall, fscore, _ = score(
            y_test, y, average=None, labels=[1, 0])
        report = build_results_report(
            f'{i}_SVM_{combination}', precision, recall, fscore)
        ...
        precision, recall, fscore, _ = score(
            y_test, y, average=None, labels=[1, 0])
        report = build_results_report(
            f'{i}_KNN_{combination}', precision, recall, fscore)
        ...
        precision, recall, fscore, _ = score(
            y_test, y, average=None, labels=[1, 0])
        report = build_results_report(
            f'{i}_RFC_{combination}', precision, recall, fscore)
